        ...
        <v-window v-model="tabs">
          ...
          <v-window-item :value="1" class="p-3">
            <v-card
              ...
              :key="index"
              :style="'border-color:' + getStatusColor(store.approval_status)"
            >
              <v-layout>
...
export default {
  ...
  methods: {
    ...
    fetchStoreDetails() {
      this.loader = true;
      this.$axios
        .get(process.env.VUE_APP_API_URL_ADMIN + "fetch-stores-details", {
          params: {
    ...
    updateApprovalStatus(comment_en = "", comment_ar = "") {
      this.loader = true;
      this.$axios
        .post(process.env.VUE_APP_API_URL_ADMIN + "update-stores-approval", {
