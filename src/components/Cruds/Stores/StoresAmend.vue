<template>
  <div class="mx-2 mt-3 p-0">
    <div class="my-3 p-0" v-bind:class="[sel_lang == 'ar' ? 'rtl-page-title' : '']">
      <page-title class="col-md-4 ml-2" :heading="$t('create_amend_stores')" :google_icon="google_icon"></page-title>
    </div>
        ...
        <v-window v-model="tabs">
          ...
          <v-window-item :value="1">
            <v-form ref="form" v-model="valid">
              <v-layout v-if="user.rolename != 'StoreAdmin'">
                <v-row class="px-6 mt-2">
                  <v-col xs="12" md="12" lg="12">
                    <!-- {{user.rolename}} -->
                    <v-radio-group v-model="stores[0].stor_type" inline class="radio_item" :disabled="$route.query.slug"
                      @change="updateType(stores[0].stor_type)">
                      <v-radio :label="$t('mall')" value="Mall" v-if="user.rolename === 'SuperUser'"></v-radio>
                      <v-radio value="Store" :label="$t('store')" v-if="user.rolename == 'SuperUser' ||
                  ...
                  <v-col cols="12" xs="12" sm="12" md="6">
                    <v-tooltip :text="this.$t('category')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-autocomplete v-bind="props" v-model="stores[0].categories" @update:modelValue="(value) => updateCategories(value)
                          " :rules="fieldRulesArray" v-bind:label="$t('category')" variant="outlined" density="compact"
                          class="required_field" required index="id" multiple :items="categories_en"
                          item-value="header_id" item-title="name"></v-autocomplete>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col xs="12" sm="12" md="6" v-if="user.rolename != 'StoreAdmin'">
                    <v-tooltip :text="this.$t('mall_name')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-autocomplete v-bind="props" v-model="stores[0].mall_name"
                          @update:modelValue="(value) => updateMallName(value)" :rules="fieldRules"
                          v-bind:label="$t('mall_name')" variant="outlined" density="compact"
                          :disabled="user.rolename == 'MallAdmin'" class="required_field" required index="id"
                          :items="mall_names_en" item-value="header_id" item-title="name"></v-autocomplete>
                      </template>
                    </v-tooltip>
                  </v-col>
                </v-row>
              ...
              <v-layout>
                <v-row class="px-6 mt-2">
                  <v-col cols="12" xs="12" sm="6" :md="stores[0].stor_type == 'Store' ? 3 : 6">
                    <v-tooltip :text="$t('name_en')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field v-bind="props" v-model="stores[0].name" :rules="fieldRules" maxlength="100"
                          v-bind:label="$t('name_en')" class="required_field" required variant="outlined"
                  ...
                  <v-col cols="12" xs="12" sm="6" md="3">
                    <v-tooltip :text="$t('org_email_en')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field v-bind="props" @update:modelValue="(value) => updateMallEmail(value, 1)
                          " v-model="stores[0].email" :rules="[...fieldRules, ...emailRules]" maxlength="100"
                          v-bind:label="$t('org_email_en')" class="required_field" required variant="outlined"
                          density="compact"></v-text-field>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col cols="12" xs="12" sm="2" md="1">
                    <v-tooltip :text="$t('mobile_code_en')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-autocomplete v-bind:label="$t('mobile_code_en')" v-bind="props" variant="outlined"
                          density="compact" :rules="fieldRules" class="required_field" required index="id"
                          item-key="header_id" item-value="header_id" item-title="mobile_code"
                          v-model="stores[0].mobile_code" @update:modelValue="(value) => updateMobileCode(value)
                            " :items="country_array_en"></v-autocomplete>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col cols="12" xs="12" sm="4" md="2">
                    <v-tooltip :text="$t('phone_en')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field v-bind="props" @update:modelValue="(value) => updateMallPhone(value, 1)
                          " v-model="stores[0].phone" :rules="phoneRules" v-on:keypress="NumbersOnly" maxlength="12"
                          v-bind:label="$t('phone_en')" required class="required_field" variant="outlined"
                          density="compact"></v-text-field>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col cols="12" xs="12" sm="12" md="3" v-if="stores[0].stor_type == 'Store'">
                    <v-tooltip :text="$t('store_code')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-autocomplete v-bind="props" @update:modelValue="(value) => updateMallSCode(value, 1)
                          " v-model="stores[0].store_code" v-bind:label="$t('store_code')" variant="outlined"
                          density="compact" required index="id" :items="shop_codes" item-value="shop_code"
                          item-title="shop_code"></v-autocomplete>
                      </template>
                    </v-tooltip>
                  </v-col>
                </v-row>
                ...
                <v-row class="px-6 mt-2">
                  <v-col cols="12" md="3" xs="6" sm="6" lg="3">
                    <v-tooltip :text="$t('country_en')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-autocomplete v-bind:label="$t('country_en')" v-bind="props" variant="outlined"
                          density="compact" index="id" item-key="name" class="required_field" item-value="header_id"
                          item-title="name" required :rules="fieldRules" v-model="stores[0].country" @update:model-value="
                  ...
                  <v-col cols="12" md="3" xs="6" sm="6" lg="3">
                    <v-tooltip :text="$t('state_en')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-autocomplete v-bind:label="$t('state_en')" v-bind="props" variant="outlined" density="compact"
                          class="required_field" index="id" item-key="name" item-title="name" item-value="header_id"
                          required :rules="fieldRules" v-model="stores[0].state" @update:model-value="
                            fetch_cities(stores[0].state);
                          updateStateF(stores[0].state, 1);
                          " :items="state_array_en"></v-autocomplete>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col cols="12" md="3" xs="6" sm="6" lg="3">
                    <v-tooltip :text="$t('city_en')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-autocomplete v-bind:label="$t('city_en')" v-bind="props" variant="outlined" density="compact"
                          index="id" item-key="name" class="required_field" item-value="header_id" item-title="name"
                          required :rules="fieldRules" v-model="stores[0].city"
                          @update:model-value="updateCityF(stores[0].city, 1)" :items="city_array_en"></v-autocomplete>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col cols="12" md="3" lg="3" xs="6" sm="6" px-2>
                    <v-tooltip :text="$t('postcode_en')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field v-bind:label="$t('postcode_en')" :rules="postcodeRules"
                          v-on:keypress="AlphanumericAndArabic" v-bind="props" variant="outlined" class="required_field"
                          density="compact" maxlength="12" @update:modelValue="(value) => updateMallPost(value, 1)
                            " v-model="stores[0].post_code" required></v-text-field>
                      </template>
                    </v-tooltip>
                  </v-col>
                </v-row>
                ...
                <v-row class="px-6 mt-2">
                  ...
                  <v-col cols="12" xs="12" sm="12" md="4">
                    <v-tooltip :text="$t('website_en')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field v-bind="props" @update:modelValue="(value) => updateMallweb(value, 1)
                          " v-model="stores[0].website" maxlength="100" v-bind:label="$t('website_en')" required
                  ...
                  <v-col cols="12" xs="12" sm="12" md="2">
                    <v-tooltip :text="$t('sequence_en')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field v-bind="props" v-model="stores[0].seq" maxlength="5" v-on:keypress="NumbersOnly"
                          v-bind:label="$t('sequence_en')" variant="outlined" density="compact"></v-text-field>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col cols="12" xs="12" sm="12" md="6">
                    <v-tooltip :text="$t('meta_title_en')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field v-bind="props" v-model="stores[0].meta_title" :rules="fieldRules" maxlength="70"
                          class="required_field" v-bind:label="$t('meta_title_en')" required counter="true"
                          variant="outlined" density="compact"></v-text-field>
                      </template>
                    </v-tooltip>
                  </v-col>
                    ...
                    <v-tooltip :text="$t('meta_description_en')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-textarea rows="3" v-bind="props" variant="outlined" density="compact" :rules="descriptionRules"
                          maxlength="160" counter="true" class="required_field" v-model="stores[0].meta_description"
                    ...
                    <div>
                        ...
                        <v-hover v-slot="{ isHovering, props }">
                          <div style="position: relative" v-bind="props">
                            <img v-bind:style="isHovering == true ? 'filter: blur(1px);' : ''
                              " v-if="stores[0].icon != null" :src="envImagePath + stores[0].icon" width="100" height="65
                          " alt />
                            <img v-bind:style="isHovering == true ? 'filter: blur(1px);' : ''
                              " v-else src="@/assets/images/upload_image_default.png" width="100" />
                            <div v-show="isHovering" class="camera-icon">
                              <v-icon @click="uploadFile('icon')">mdi-camera</v-icon>
                            </div>
                          </div>
                        </v-hover>
                      </div>
                      <span>
                        <v-tooltip :text="this.$t('delete_en')" location="bottom">
                          <template v-slot:activator="{ props }">
                            <v-icon small v-bind="props" v-if="stores[0].icon" class="mr-2 edit_btn icon_size delete_icon"
                              @click="removeImage(0, 'icon')">mdi mdi-trash-can-outline</v-icon>
                          </template>
                        </v-tooltip>
                      </span>
                      <v-tooltip :text="this.$t('download_en')" location="bottom">
                        <template v-slot:activator="{ props }">
                          <a class="text-center pointer download_icon">
                            <span><v-icon v-if="stores[0].icon" v-bind="props" class="mr-2"
                                @click="downloadImage(stores[0].icon)">mdi mdi-download</v-icon></span>
                          </a>
                        </template>
                      </v-tooltip>
                    </div>
                    <br />
                    <Imageupload :folder="'stores'" :resizewidth="resizewidth" :resizeheight="resizeheight"
                  ...
                  <v-col cols="12" xs="12" sm="12" md="4">
                    <div>
                        ...
                        <v-hover v-slot="{ isHovering, props }">
                          <div style="position: relative" v-bind="props">
                            <img v-bind:style="isHovering == true ? 'filter: blur(1px);' : ''
                              ...
                              :src="envImagePath + stores[0].background_image" width="100" height="65
                          " alt />
                            <img v-bind:style="isHovering == true ? 'filter: blur(1px);' : ''
                              " v-else src="@/assets/images/upload_image_default.png" width="100" />
                            <div v-show="isHovering" class="camera-icon">
                              <v-icon @click="uploadFile('b_image')">mdi-camera</v-icon>
                            </div>
                          </div>
                        </v-hover>
                      </div>
                      <span>
                        <v-tooltip :text="this.$t('delete_en')" location="bottom">
                          <template v-slot:activator="{ props }">
                            <v-icon small v-bind="props" v-if="stores[0].background_image"
                              class="mr-2 edit_btn icon_size delete_icon" @click="removeImage(0, 'b_image')">mdi
                      ...
                      <v-tooltip :text="this.$t('download_en')" location="bottom">
                        <template v-slot:activator="{ props }">
                          <a class="text-center pointer download_icon">
                            <span><v-icon v-if="stores[0].background_image" v-bind="props" class="mr-2" @click="
                    ...
                    <br />
                    <Imageupload :folder="'stores'" :resizewidth="resizewidth" :resizeheight="resizeheight"
                      @uploaded_image="uploaded_image" :upload_profile="upload_bi_file" />
                  </v-col>
                </v-row>
              </v-layout>
            </v-form>
          ...
          <v-window-item :value="2">
            <v-form ref="form" v-model="validAR" class="rtl-direction">
              <v-layout v-if="user.rolename != 'StoreAdmin'">
                <v-row class="px-6 mt-2">
                  <v-col xs="12" sm="12" md="12" lg="12">
                    <v-radio-group v-model="stores[1].stor_type" inline style="direction: rtl" class="radio_item"
                      :disabled="$route.query.slug" @change="updateType(stores[1].stor_type)">
                      <v-radio :label="$t('mall_ar')" value="Mall" v-if="user.rolename === 'SuperUser'"></v-radio>
                      <v-radio value="Store" :label="$t('store_ar')" v-if="user.rolename == 'SuperUser' ||
                        user.rolename == 'MallAdmin'
                        "></v-radio>
                    </v-radio-group>
                  </v-col>
                ...
                <v-row class="px-6 mt-2 arabdirection">
                  <v-col xs="12" sm="12" md="6">
                    <v-tooltip :text="this.$t('category_ar')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-autocomplete v-bind="props" v-model="stores[1].categories" @update:modelValue="(value) => updateCategories(value)
                          " :rules="fieldRulesAr" v-bind:label="$t('category_ar')" variant="outlined" density="compact"
                          class="required_field rtl" required index="id" multiple :items="categories_ar"
                          item-value="header_id" item-title="name"></v-autocomplete>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col xs="12" sm="12" md="6" v-if="user.rolename != 'StoreAdmin'">
                    <v-tooltip :text="this.$t('mall_name')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-autocomplete v-bind="props" v-model="stores[1].mall_name"
                          @update:modelValue="(value) => updateMallName(value)" :rules="fieldRulesAr"
                          v-bind:label="$t('mall_name_ar')" variant="outlined" density="compact"
                          class="required_field rtl" :disabled="user.rolename == 'MallAdmin'" required index="id"
                          :items="mall_names_ar" item-value="header_id" item-title="name"></v-autocomplete>
                      </template>
                    </v-tooltip>
                  </v-col>
                </v-row>
                ...
                <v-row class="px-6 mt-2 arabdirection">
                  <v-col cols="12" xs="12" sm="12" :md="stores[0].stor_type == 'Store' ? 3 : 6">
                    <v-tooltip :text="$t('name_ar')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field v-bind="props" v-model="stores[1].name" :rules="fieldRulesAr" maxlength="100"
                          v-bind:label="$t('name_ar')" required variant="outlined" density="compact"
                          class="required_field rtl"></v-text-field>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col cols="12" xs="12" sm="12" md="3">
                    <v-tooltip :text="$t('org_email_ar')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field @update:modelValue="(value) => updateMallEmail(value, 0)
                          " v-bind="props" v-model="stores[1].email" :rules="[...fieldRulesAr, ...emailRulesAr]"
                          maxlength="100" v-bind:label="$t('org_email_ar')" required variant="outlined" density="compact"
                          class="required_field rtl"></v-text-field>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col cols="12" xs="12" sm="2" md="1">
                    <v-tooltip :text="$t('mobile_code_ar')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-autocomplete v-bind:label="$t('mobile_code_ar')" v-bind="props" variant="outlined"
                          density="compact" :rules="fieldRulesAr" class="required_field" required index="id"
                          item-key="header_id" item-value="header_id" item-title="mobile_code"
                          v-model="stores[1].mobile_code" @update:modelValue="(value) => updateMobileCode(value)
                            " :items="country_array_ar"></v-autocomplete>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col cols="12" xs="12" sm="4" md="2">
                    <v-tooltip :text="$t('phone_ar')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field v-bind="props" @update:modelValue="(value) => updateMallPhone(value, 0)
                          " v-model="stores[1].phone" :rules="phoneRulesAr" v-on:keypress="NumbersOnly" maxlength="12"
                          v-bind:label="$t('phone_ar')" required variant="outlined" density="compact"
                          class="required_field rtl"></v-text-field>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col cols="12" xs="12" sm="12" md="3" v-if="stores[0].stor_type == 'Store'">
                    <v-tooltip :text="$t('store_code_ar')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-autocomplete v-bind="props" @update:modelValue="(value) => updateMallSCode(value, 0)
                          " v-model="stores[1].store_code" v-bind:label="$t('store_code_ar')" variant="outlined"
                          density="compact" required counter="true" index="id" :items="shop_codes" item-value="shop_code"
                          item-title="shop_code"></v-autocomplete>
                      </template>
                    </v-tooltip>
                  </v-col>
                ...
                <v-row class="px-6 mt-2 arabdirection">
                  <v-col cols="12" md="3" xs="6" sm="6" lg="3">
                    <v-tooltip :text="$t('country_ar')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-autocomplete v-bind:label="$t('country_ar')" v-bind="props" variant="outlined"
                          density="compact" index="id" item-key="name" item-value="header_id" class="required_field rtl"
                          item-title="name" required :rules="fieldRulesAr" v-model="stores[1].country"
                          @update:model-value="
                  ...
                  <v-col cols="12" md="3" xs="6" sm="6" lg="3">
                    <v-tooltip :text="$t('state_ar')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-autocomplete v-bind:label="$t('state_ar')" v-bind="props" variant="outlined" density="compact"
                          index="id" class="required_field rtl" item-key="name" item-value="header_id" item-title="name"
                          required :rules="fieldRulesAr" v-model="stores[1].state" @update:model-value="
                            fetch_cities(stores[1].state, 0);
                          updateStateF(stores[1].country, 0);
                          " :items="state_array_ar"></v-autocomplete>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col cols="12" md="3" xs="6" sm="6" lg="3">
                    <v-tooltip :text="$t('city_ar')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-autocomplete v-bind:label="$t('city_ar')" v-bind="props" variant="outlined" density="compact"
                          index="id" item-key="name" item-title="name" class="required_field rtl" required
                          :rules="fieldRulesAr" item-value="header_id" v-model="stores[1].city"
                          @update:model-value="updateCityF(stores[1].city, 0)" :items="city_array_ar"></v-autocomplete>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col cols="12" md="3" lg="3" xs="6" sm="6" px-2>
                    <v-tooltip :text="$t('postcode_ar')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field v-bind:label="$t('postcode_ar')" :rules="postcodeRulesAr"
                          v-on:keypress="AlphanumericAndArabic" v-bind="props" variant="outlined" density="compact"
                          maxlength="12" class="required_field rtl" @update:modelValue="(value) => updateMallPost(value, 0)
                            " v-model="stores[1].post_code" required></v-text-field>
                      </template>
                    </v-tooltip>
                  </v-col>
                </v-row>
                ...
                <v-row class="px-6 mt-2 arabdirection">
                  <v-col cols="12" xs="12" sm="12" md="4">
                    <v-tooltip :text="$t('website_ar')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field v-bind="props" @update:modelValue="(value) => updateMallweb(value, 0)
                          " v-model="stores[1].website" maxlength="100" v-bind:label="$t('website_ar')" required
                          counter="true" variant="outlined" density="compact"></v-text-field>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col cols="12" xs="12" sm="12" md="2">
                    <v-tooltip :text="$t('sequence_ar')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field v-bind="props" v-model="stores[1].seq" v-on:keypress="NumbersOnly" maxlength="5"
                          v-bind:label="$t('sequence_ar')" class="rtl" variant="outlined"
                          density="compact"></v-text-field>
                      </template>
                    </v-tooltip>
                  </v-col>
                  <v-col cols="12" xs="12" sm="12" md="6">
                    <v-tooltip :text="$t('meta_title_ar')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field v-bind="props" v-model="stores[1].meta_title" :rules="fieldRulesAr" maxlength="70"
                          v-bind:label="$t('meta_title_ar')" required counter="true" class="required_field rtl"
                          variant="outlined" density="compact"></v-text-field>
                      </template>
                    </v-tooltip>
                  </v-col>
                    ...
                    <div>
                      ...
                      <div class="image-container">
                        <v-hover v-slot="{ isHovering, props }">
                          <div style="position: relative" v-bind="props">
                            <img v-bind:style="isHovering == true ? 'filter: blur(1px);' : ''
                              " v-if="stores[1].icon != null" :src="envImagePath + stores[1].icon" width="100" height="65
                          " alt />
                            <img v-bind:style="isHovering == true ? 'filter: blur(1px);' : ''
                              " v-else src="@/assets/images/upload_image_default.png" width="100" />
                            <div v-show="isHovering" class="camera-icon">
                      ...
                      <span>
                        <v-tooltip :text="this.$t('delete_ar')" location="bottom">
                          <template v-slot:activator="{ props }">
                            <v-icon v-bind="props" small v-if="stores[1].icon"
                              class="mr-2 edit_btn icon_size delete_icon_ar" @click="removeImage(1, 'icon')">mdi
                              mdi-trash-can-outline</v-icon>
                          </template>
                        </v-tooltip>
                      </span>
                      ...
                      <v-tooltip :text="this.$t('download_ar')" location="bottom">
                        <template v-slot:activator="{ props }">
                          <a class="text-center pointer download_icon_ar">
                            <span><v-icon v-if="stores[1].icon" v-bind="props" class="mr-2"
                                @click="downloadImage(stores[1].icon)">mdi mdi-download</v-icon></span>
                          </a>
                        </template>
                      </v-tooltip>
                    </div>
                  ...
                  <v-col cols="12" xs="12" sm="12" md="4">
                    <div>
                        ...
                        <v-hover v-slot="{ isHovering, props }">
                          <div style="position: relative" v-bind="props">
                            <img v-bind:style="isHovering == true ? 'filter: blur(1px);' : ''
                              ...
                              :src="envImagePath + stores[1].background_image" width="100" height="65
                          " alt />
                            <img v-bind:style="isHovering == true ? 'filter: blur(1px);' : ''
                              " v-else src="@/assets/images/upload_image_default.png" width="100" />
                            <div v-show="isHovering" class="camera-icon">
                      ...
                      <span>
                        <v-tooltip :text="this.$t('delete_ar')" location="bottom">
                          <template v-slot:activator="{ props }">
                            <v-icon small v-bind="props" v-if="stores[1].background_image"
                              class="mr-2 edit_btn icon_size delete_icon_ar" @click="removeImage(1, 'b_image')">mdi
                              mdi-trash-can-outline</v-icon>
                          </template>
                        </v-tooltip>
                      </span>
                      <v-tooltip :text="this.$t('download_ar')" location="bottom">
                        <template v-slot:activator="{ props }">
                          <a class="text-center pointer download_icon_ar">
                            <span><v-icon v-if="stores[1].background_image" v-bind="props" class="mr-2" @click="
                    ...
                    <br />
                    <Imageupload :folder="'stores'" :resizewidth="resizewidth" :resizeheight="resizeheight"
                      @uploaded_image="uploaded_image" :upload_profile="upload_bi_file_ar" />
                  </v-col>
                </v-row>
              </v-layout>
            </v-form>
      ...
      <div v-if="social_media_en.length > 0 && social_media_ar.length > 0">
        ...
        <v-layout>
          <v-row class="px-6 mt-2" v-if="tabs == 1">
            <v-col v-for="(media_data, media_index) in social_media_en" :key="media_index" cols="12" sm="4" md="4">
              <v-tooltip :text="media_data.longname" location="bottom">
                <template v-slot:activator="{ props }">
                  <v-text-field v-if="social_media[media_index]" v-bind="props" v-model="social_media[media_index].link"
                    maxlength="100" v-bind:label="media_data.longname" required variant="outlined"
                    density="compact"></v-text-field>
                </template>
              </v-tooltip>
            </v-col>
          ...
          <v-row class="px-6 mt-2 arabdirection" v-else>
            <v-col v-for="(media_data, media_index) in social_media_ar" :key="media_index" cols="12" sm="4" md="4">
              <v-tooltip :text="media_data.longname" location="bottom">
                <template v-slot:activator="{ props }">
                  <v-text-field v-bind="props" v-if="social_media[media_index]" v-model="social_media[media_index].link"
                    maxlength="100" v-bind:label="media_data.longname" required variant="outlined"
                    density="compact"></v-text-field>
                </template>
              </v-tooltip>
